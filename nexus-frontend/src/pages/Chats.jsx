...
const Chats = () => {
  ...
  useEffect(() => {
    if (activeChat && user) {
      ...
      const websocket = new WebSocket(
        `wss://https://nexus-test-api-8bf398f16fc4.herokuapp.com/chats/ws/chat/?access_token=${accessToken}`
      );
  ...
  const fetchConversations = async () => {
    try {
      ...
      const groups = groupResponse.data.map((group) => ({
        ...
        lastMessage: group.last_message_timestamp
          ? `Last message at ${new Date(
  ...
  const fetchMessages = async (conversation) => {
    try {
      ...
      setError(
        `Failed to load messages: ${err.response?.status || err.message}`
      );
    }
  };
    ...
    try {
      ...
      setError(
        `Failed to upload ${file.name}: ${
          err.response?.data?.detail || err.response?.data?.file || err.message
        }`
      );
      setUploadingFiles((prev) => prev.filter((f) => f.id !== fileId));
    }
  ...
  const renderMessage = (msg) => {
    ...
    switch (msg.type) {
      ...
      case "file":
        ...
        return (
          <div
            className="message-file document-message"
            ...
            <div className="file-info">
              <div className="file-header">
                ...
                <span className="file-type-label">
                  {getFileTypeLabel(msg.fileData.type)}
                </span>
              </div>
              <span className="file-size">{msg.fileData.size}</span>
  ...
  const handleVideoCall = () => {
    if (activeChat) {
      navigate(
        `/video-call?participant=${encodeURIComponent(activeChat.name)}&id=${
          activeChat.id
        }`
      );
    }
  ...
  const handleAudioCall = () => {
    if (activeChat) {
      navigate(
        `/audio-call?participant=${encodeURIComponent(activeChat.name)}&id=${
          activeChat.id
        }`
      );
    }
    ...
    <div className="chats-page">
      ...
      <div className={`chat-main ${!showMobileChat ? "mobile-hidden" : ""}`}>
              ...
              {(messages[activeChat.id] || []).map((msg) => {
                ...
                return (
                  <div
                    key={msg.id}
                    className={`message ${
                      msg.senderId === user.chat_user_id ? "own-message" : ""
                    }`}
                  >
                    {msg.senderId !== user?.id && (
                ...
                <div className="file-upload-options">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      const fileType = fileInputRef.current.accept.includes(
                        "image"
                      )
                        ? "image"
              ...
              <div className="image-modal-header">
                ...
                <div className="image-modal-actions">
                  ...
                  <button
                    ...
                    onClick={() =>
                      setImageModal({ isOpen: false, src: "", name: "" })
                    }
                    title="Close"
                  >
                    <X size={20} />
        ...
        {showGroupLinkModal && activeChat && activeChat.type === "group" && (
          <div
            className="link-group-link-modal-overlay"
            onClick={() => setShowGroupLinkModal(false)}
          >
            <div
              className="link-group-link-modal-content"
              onClick={(e) => e.stopPropagation()}
            >
              ...
              <div className="link-group-link-modal-body">
                ...
                <div className="link-link-section">
                  ...
                  <div className="link-link-input-container">
                    <input
                      type="text"
                      value={activeChat.groupLink || ""}
                      readOnly
                      className="group-link-input"
                    />
                    <button
                      className={`link-copy-link-btn ${
                        linkCopied ? "copied" : ""
                      }`}
